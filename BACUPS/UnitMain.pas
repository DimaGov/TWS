(*     +-------------------------------------------------------------------+
 ...
 |                                                                   |     +
 |    UKRAINE, DNEPR CITY, 2017-2019 (C)                             |    /
 |                                                                   |   /
 |    zdsimulator.com.ua                                             |  /
...
uses
  Windows, Messages, SysUtils, Graphics, Forms, Dialogs, StdCtrls, ComCtrls, Bass, inifiles,
  ExtCtrls, UnitAuthors, Controls, TlHelp32, ShellApi, Grids, ValEdit, jpeg, UnitSAVPEHelp,
  IdBaseComponent, IdCoder, IdCoder3to4, Menus, IdCoderMIME, Classes, UnitSettings, UnitDebug, Math,
  UnitUSAVP, EncdDecd, SAVP, RAMMemModule, FileManager, ExtraUtils, SoundManager, Debug, bass_fx;
...
type
  TFormMain = class(TForm)
    ...
    InformatorDoorCloseDelay: TTimer;
    ...
    procedure InformatorDoorCloseDelayTimer(Sender: TObject);
    ...
    procedure TimerDebugTickTimer(Sender: TObject);
...
var
  ...
  // -=|=- ДАННЫЕ ПОЛУЧАЕМЫЕ С ФАЛЙА settings.ini -=|=- //
  ...
  SceneryName:                 String;
  LocoNum:                     Integer;
  LocoPowerVoltage:            Integer;
  // -=|=-=|=-=|=-=|=-=|=-=|=-=|=-=|=-=|=-=|=-=|=-=|=- //

  // === Переменные для хранения границ станций из файла start_kilometers.dat === //
...
//------------------------------------------------------------------------------//
procedure TFormMain.cbWagPerestukClick(Sender: TObject);	// Нажатие на "Перестук вагонов"
begin
  if cbWagPerestuk.Checked=True then begin
    Panel1.Enabled:=True;
    WagF:='';
    FormMain.ClientHeight:=FormMain.ClientHeight+Panel1.Height;
  end else begin
...
begin
  if cbSAUTSounds.Checked=True then begin
     ...
     SAUTOFFF:='TWS/INFO/USAVPP_25KV_podskazka.mp3';SAUTOff:=True;isSpeedLimitRouteLoad:=False;
     SAVPName := 'SAUT';
     Load_TWS_SAVP_EK();
  end else begin
     ...
     SAUTOFFF:='TWS/INFO/SAUT_Off.mp3';
...
begin
  ...
  if (cbPRS_RZD.Checked=True) or (cbPRS_UZ.Checked=True) then Timer3.Enabled:=True;
end;
...
begin
  if cbUSAVPSounds.Checked=True then begin
     //FormUSAVP.Show;
     USAVPEnabled := True;
     cbSAUTSounds.Checked:=False;cbGSAUTSounds.Checked:=False;cbSAVPESounds.Checked:=False;cbEPL2TBlock.Checked:=False;
     SAUTOFFF:='TWS/INFO/USAVPP_25KV_podskazka.mp3'; isSpeedLimitRouteLoad:=False;SAUTOff:=True;
     SAVPName := 'USAVPP';
     Load_TWS_SAVP_EK();
  end else begin
    USAVPEnabled := False;
...
begin
  ...
  LoadTWSParams('TWS\settings_TWS.ini');   				     // Делаем автозагрузку

  isGameOnPause := True;

  MainCycleFreq := Timer1.Interval;
...
try
  ...
  if cbCabinClicks.Checked=True then begin
     ...
     if LocoWithSndKM = True then begin
        ...
        if ReversorPos<>0 then begin
           if LocoWithSndKM_OP = True then begin
           ...
           if (KM_OP+getasynckeystate(16)=0) and (Loco <> 'ED4M') and (LocoGlobal<>'M62') then begin
              // -/- A -/- //
  ...
  if (cbSignalsSounds.Checked = True) and (Track <> 0) then begin
     ...
     //SvistokTimeLeft := BASS_ChannelBytes2Seconds(SvistokChannel, BASS_ChannelGetLength(SvistokChannel, BASS_POS_BYTE) - BASS_ChannelGetPosition(SvistokChannel, BASS_POS_BYTE));
     ...
     //TifonTimeLeft := BASS_ChannelBytes2Seconds(TifonChannel, BASS_ChannelGetLength(TifonChannel, BASS_POS_BYTE) - BASS_ChannelGetPosition(TifonChannel, BASS_POS_BYTE));
  ...
  if cbVspomMash.Checked=True then begin
      ...
      if (LocoGlobal='CHS7') Or (LocoGlobal='CHS8') Or (LocoGlobal='CHS4t') then begin
         ...
         if (LocoGlobal='CHS7') and ((KM_Pos_1 <> Prev_KMAbs) Or (BV <> PrevBV) Or (Voltage <> PrevVoltage)) then begin
            ...
            if ((I = 0) Or (StopVentTD = True)) and (BV <> 0) and (Voltage <> 0) then begin
               if (KM_Pos_1 in [1..17]) Or (KM_Pos_1 in [21..35]) Or (KM_Pos_1 in [39..53]) then begin
                  VentTDF := PChar('TWS\CHS7_8\mvPTR_start.wav'); VentCycleTDF := PChar('TWS\CHS7_8\mvPTR_loop.wav');
                  XVentTDF:= PChar('TWS\CHS7_8\x_mvPTR_start.wav');XVentCycleTDF:=PChar('TWS\CHS7_8\x_mvPTR_loop.wav');
            ...
            if (I <> 0) and (StopVentTD = False) then begin
               ...
               then begin
                  VentTDF := PChar('TWS\CHS7_8\mvPTR_stop.wav'); VentCycleTDF := PChar('');
                  XVentTDF:= PChar('TWS\CHS7_8\x_mvPTR_stop.wav');XVentCycleTDF:=PChar('');
         ...
         if (LocoGlobal='CHS8') and (CHS8VentStarted = False) then begin
            VentTDF := PChar('TWS\CHS7_8\chs8_vent.wav'); VentCycleTDF := PChar('TWS\CHS7_8\chs8_vent.wav');
            XVentTDF := PChar('TWS\CHS7_8\x_chs8_vent.wav');XVentCycleTDF:=PChar('TWS\CHS7_8\x_chs8_vent.wav');
      ...
      if Compressor<>Prev_Compressor then begin
         if Compressor<>0 then begin
            if LocoGlobal='CHS7' then begin
               CompressorF:=PChar('TWS/CHS7_8/mk-start-chs7.wav'); CompressorCycleF:=PChar('TWS/CHS7_8/mk-loop-chs7.wav');
               XCompressorF:=PChar('TWS/CHS7_8/x_mk-start-chs7.wav'); XCompressorCycleF:=PChar('TWS/CHS7_8/x_mk-loop-chs7.wav');
            end;
            if LocoGlobal='CHS8' then begin
               CompressorF:=PChar('TWS/CHS7_8/mk-start-chs8.wav'); CompressorCycleF:=PChar('TWS/CHS7_8/mk-loop-chs8.wav');
               XCompressorF:=PChar('TWS/CHS7_8/x_mk-start-chs8.wav'); XCompressorCycleF:=PChar('TWS/CHS7_8/x_mk-loop-chs8.wav');
            end;
         ...
         if Compressor=0 then begin
            if LocoGlobal='CHS7' then begin
               CompressorF:=PChar('TWS/CHS7_8/mk-stop-chs7.wav');
               XCompressorF:=PChar('TWS/CHS7_8/x_mk-stop-chs7.wav');
            end;
            if LocoGlobal='CHS8' then begin
               CompressorF:=PChar('TWS/CHS7_8/mk-stop-chs8.wav');
               XCompressorF:=PChar('TWS/CHS7_8/x_mk-stop-chs8.wav');
            end;
      ...
      if (LocoGlobal = 'EP1m') Or (LocoGlobal = '2ES5K') then begin
         if LocoGlobal = '2ES5K' then Voltage := 25;
         ...
         if (Vent + Vent2 + Vent3 = 0) Or (Voltage <= 20) Or (Vent + Vent2 + Vent3 < Prev_Vent + Prev_Vent2 + Prev_Vent3) then begin
            if ((Vent = 0) and (Prev_Vent = 1) and (Voltage > 20)) Or
               ((Vent2 = 0) and (Prev_Vent2 = 1) and (Voltage > 20)) Or
               ((Vent3 = 0) and (Prev_Vent3 = 1) and (Voltage > 20)) then begin
      ...
      if LocoGlobal='CHS8' then begin
         if ((PrevTEDAmperage=0) and (TEDAmperage>0)) or
            ...
            TWS_PlayUnipuls(PChar('TWS/CHS7_8/Unipuls/50-300 A.wav'), True);
            UnipulsFaktPos:=0;
         end;
         ...
         if ((PrevTEDAmperage>0) and (TEDAmperage=0) and (EDTAmperage=0)) then begin
             UnipulsFaktVol:=trcBarVspomMahVol.Position; UnipulsTargetVol:=0; isStopUnipuls:=True; end;
         if ((TEDAmperage=0) and (TimerPerehodUnipulsSwitch.Enabled = False) and (isStopUnipuls=False) and (EDTAmperage=0)) then begin
         ...
         if (TimerPerehodUnipulsSwitch.Enabled = False) and (UnipulsFaktPos<>UnipulsTargetPos) then begin
             ...
             If UnipulsFaktPos = 0 then begin
                TWS_PlayUnipuls(PChar('TWS/CHS7_8/Unipuls/50-300 A.wav'), True); end;
             If UnipulsFaktPos = 1 then begin
                TWS_PlayUnipuls(PChar('TWS/CHS7_8/Unipuls/300-400 A.wav'), True); end;
             If UnipulsFaktPos = 2 then begin
                TWS_PlayUnipuls(PChar('TWS/CHS7_8/Unipuls/400-500 A.wav'), True); end;
             If UnipulsFaktPos = 3 then begin
                TWS_PlayUnipuls(PChar('TWS/CHS7_8/Unipuls/500-600 A.wav'), True); end;
             If UnipulsFaktPos = 4 then begin
                TWS_PlayUnipuls(PChar('TWS/CHS7_8/Unipuls/600-700 A.wav'), True); end;
             If UnipulsFaktPos = 5 then begin
                TWS_PlayUnipuls(PChar('TWS/CHS7_8/Unipuls/700-800 A.wav'), True); end;
             If UnipulsFaktPos = 6 then begin
                TWS_PlayUnipuls(PChar('TWS/CHS7_8/Unipuls/800-900 A.wav'), True); end;
             If UnipulsFaktPos = 7 then begin
                TWS_PlayUnipuls(PChar('TWS/CHS7_8/Unipuls/900-1000 A.wav'), True); end;
             If UnipulsFaktPos = 8 then begin
                TWS_PlayUnipuls(PChar('TWS/CHS7_8/Unipuls/1000-1200 A.wav'), True); end;
             If UnipulsFaktPos = 10 then begin
                TWS_PlayUnipuls(PChar('TWS/CHS7_8/Unipuls/1200-~.wav'), True); end;
         end;
     end;
     // ********************* //
  end;
  ...
  if cbLocPerestuk.Checked=True then begin
    ...
    if (cbWagPerestuk.Checked=True) and (CoupleStat<>0) then begin
       if RadioButton2.Checked = True then begin
          if Abs(PrevAcceleretion)<0.01 then begin
             if (Acceleretion>0) and (Speed<1) and (KM_Pos_1<>0) then begin
...
PrevConMem:=isConnectedMemory;
   ...
   //On E : Exception do
      //DebugWriteErrorToErrorList('Err:1,m:' + E.Message);
end;
end;
...
begin
    try BASS_ChannelStop(SvistokChannel); BASS_StreamFree(SvistokChannel);
       ...
       if Camera=0 then BASS_ChannelSetAttribute(SvistokChannel, BASS_ATTRIB_VOL, trcBarSignalsVol.Position/120);
       if Camera=1 then BASS_ChannelSetAttribute(SvistokChannel, BASS_ATTRIB_VOL, trcBarSignalsVol.Position/100);
       if Camera=2 then BASS_ChannelSetAttribute(SvistokChannel, BASS_ATTRIB_VOL, (5 / (WagsNum + LocoSectionsNum)) * (trcBarSignalsVol.Position / 100));
       //BASS_ChannelStop(SvistokCycleChannel); BASS_StreamFree(SvistokCycleChannel);
       BASS_ChannelSetSync(SvistokChannel, BASS_SYNC_POS,
                           ...
                           //BASS_ChannelGetLength(SvistokChannel, BASS_POS_BYTE),
...
begin
    try BASS_ChannelStop(TifonChannel); BASS_StreamFree(TifonChannel);
       ...
       if Camera=0 then BASS_ChannelSetAttribute(TifonChannel, BASS_ATTRIB_VOL, trcBarSignalsVol.Position/120);
       if Camera=1 then BASS_ChannelSetAttribute(TifonChannel, BASS_ATTRIB_VOL, trcBarSignalsVol.Position/100);
       if Camera=2 then BASS_ChannelSetAttribute(TifonChannel, BASS_ATTRIB_VOL, (5 / (WagsNum + LocoSectionsNum)) * (trcBarSignalsVol.Position / 100));
       //BASS_ChannelStop(TifonCycleChannel); BASS_StreamFree(TifonCycleChannel);
...
begin
    try BASS_ChannelStop(TifonCycleChannel); BASS_StreamFree(TifonCycleChannel);
       ...
       //if Camera=0 then BASS_ChannelSetAttribute(TifonCycleChannel, BASS_ATTRIB_VOL, trcBarSignalsVol.Position/120);
       //if Camera=1 then BASS_ChannelSetAttribute(TifonCycleChannel, BASS_ATTRIB_VOL, trcBarSignalsVol.Position/100);
       //if Camera=2 then BASS_ChannelSetAttribute(TifonCycleChannel, BASS_ATTRIB_VOL, (5 / (WagsNum + LocoSectionsNum)) * (trcBarSignalsVol.Position / 100));
...
begin
  ...
  if (isPlayPerestuk_OnStation=True) then Timer3.Interval:=180000 else begin
    Randomize; Randomize; Timer3.Interval:=350000+Random(150000); end;
end;
...
begin
   if cbEPL2TBlock.Checked=True then begin
      ...
      GroupBox3.Left:=GroupBox3.Left+GroupBox8.Width;
      GroupBox2.Left:=GroupBox2.Left+GroupBox8.Width;
      GroupBox4.Left:=GroupBox4.Left+GroupBox8.Width;
      Label16.Left:=Label16.Left+GroupBox8.Width;
      Label5.Left:=Label5.Left+GroupBox8.Width;
      Panel1.Left:=Panel1.Left+GroupBox8.Width;
      GroupBox8.Visible:=True;
      FormMain.ClientWidth:=FormMain.ClientWidth+GroupBox8.Width;
...
begin
	if cbSAVPESounds.Checked=True then begin
          ...
          GroupBox3.Left:=GroupBox3.Left+SAVPEBox.Width;
          GroupBox2.Left:=GroupBox2.Left+SAVPEBox.Width;
          GroupBox4.Left:=GroupBox4.Left+SAVPEBox.Width;
          Label16.Left:=Label16.Left+SAVPEBox.Width;
          Label5.Left:=Label5.Left+SAVPEBox.Width;
          Panel1.Left:=Panel1.Left+SAVPEBox.Width;
          SAVPEBox.Visible := True;
          FormMain.ClientWidth:=FormMain.ClientWidth+SAVPEBox.Width;
...
procedure TFormMain.InformatorDoorCloseDelayTimer(Sender: TObject);
begin
...
procedure TFormMain.TimerDebugTickTimer(Sender: TObject);
begin
 ;
